...
function forceToBigint(number: string | number) {
  const str = String(number).split('.')[0]
  try {
    return BigInt(str)
  } catch {
...
const safeHexToDecimal = (hex: Hex): bigint => BigInt(hex)
const safeDecimalToHex = (input: bigint): Hex =>
  `0x${BigInt(input)
    .toString(16) /** @dev force 62-char + 0x format */
    .padStart(62, '0')}`

const assertIsHex = (value: string | object): value is Hex => typeof value === 'string' && value.startsWith('0x')
